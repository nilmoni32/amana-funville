...
<head>
    ...
    <title>Ecom Profit-Loss Report(summary) - {{ config('app.name', 'Funville') }}</title>
    <style>
        body{
    ...
    <div class="content-wrapper">
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                <h6 style="margin-top:5px; text-align: left;">Report: E-Commerce Profit Loss Report ( Summary ) </h6>
        ...
        <div class="invoice-description">
            <div class="invoice-left-top float-left" style="width:100%;">
                ...
                @php $totalSalesCost=0.0; @endphp
                @foreach($time_sales as $cart)
                @php $totalSalesCost += $cart->salesCost;@endphp
                @endforeach
                ...
                <span>Net Sales Cost</span><span style="padding-left: 74px;">:</span>  {{ round($totalSalesCost,2) }} {{ config('settings.currency_symbol') }}<br/>
                <span>Net Profit/Loss</span><span style="padding-left: 76px;">:</span>  {{ round(($total_sales - $totalSalesCost),2)  }} {{ config('settings.currency_symbol') }} <br/>
                </p> --}}
            </div>
            <div class="clearfix"></div>
        </div>
                ...
                <tbody>           
                    <tr>
                        ...
                        <td class="text-left">{{ round($totalSalesCost,2)  }}
                            {{ config('settings.currency_symbol') }}</td>
                        <td class="text-left">{{ round(($total_sales - $totalSalesCost) ,2)  }}
                            {{ config('settings.currency_symbol') }} &nbsp; {{ "( " . round((($total_sales - $totalSalesCost) / $totalSalesCost)*100,2) ."% )" }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
